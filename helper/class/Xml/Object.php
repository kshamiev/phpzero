...
class Helper_Xml_Object
{
    ...
    public function Set_Data($data, $flag = 1)
    {
        if ( 0 < $flag )
        {
            $this->_Data = Helper_Xml_Parser::String_Xml($data);
        }
        else
        {
            $this->_Data = Helper_Xml_Parser::String_Xml_NotHtml($data);
        }
    }

    /**
     * @param $name
     * @return Helper_Xml_Object
     */
    ...
    /**
     * @param $name
     * @return array Helper_Xml_Object
     */
    ...
    public function Node_Add(Helper_Xml_Object $Helper_Xml_Object)
    {
        $this->_Node[$Helper_Xml_Object->_Name][] = $Helper_Xml_Object;
    }

    /**
     * Поиск нод по имени в передаваемом Xml объекте.
     *
     * @param Helper_Xml_Object $Helper_Xml_Object - Xml объект в котором производится поиск
     * @param string $name - имя ноды которую ищем
     * @return Helper_Xml_Object - найденая нода/список нод либо null
     */
    public static function Search_Nodes(Helper_Xml_Object $Helper_Xml_Object, $name)
    {
        if ( !is_null($Xml = $Helper_Xml_Object->Get_Nodes($name)) )
        {
            return $Xml;
        }
        else
        {
            foreach ($Helper_Xml_Object->Get_NodeTags() as $xml_list)
            {
     ...
     *
     * @param string $name - имя ноды
     * @return Helper_Xml_Object - созданная дочерняя нода.
     */
    public function Node_Create($name)
    {
        $Xml_Node = new Helper_Xml_Object($name);
