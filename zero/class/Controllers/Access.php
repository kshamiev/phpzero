...
class Zero_Controllers_Access extends Zero_Controller
{
    ...
    protected function Chunk_View()
    {
        ...
        $this->View->Assign('groups', Zero_DB::Select_List_Index("SELECT ID, `Name` FROM Groups ORDER BY `Name`"));

        $access = [];
        $result = Zero_DB::Select_Array("SELECT Groups_ID, Action FROM Action  WHERE Controllers_ID = {$this->Params['obj_parent_id']}");
