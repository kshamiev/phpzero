...
class Zero_Cache
{
    ...
    public static function Set_Data($index, $value, $time = 0)
    {
        ...
        {
            $index = ZERO_PATH_CACHE . '/' . $index . '.data';
            Zero_System_File::File_Save($index, serialize($value));
        }
        //  Memcache
        else
        {
        ...
        {
            $path = ZERO_PATH_CACHE . '/' . $arr[0] . '/' . Zero_System_File::Get_Path_Cache($arr[1]) . '/' . $arr[1] . '/cache.cache';
            Zero_System_File::File_Save_After($path, $index);
        }
        self::$_Link = [];
        return true;
    }
    ...
    {
        $path_file = ZERO_PATH_CACHE . '/' . $this->Model->Source . '/' . Zero_System_File::Get_Path_Cache($this->Model->ID) . '/' . $this->Model->ID . '/cache.cache';
    ...
    public static function Reset_All()
    {
        ...
        //  file cache
        Zero_System_File::File_Remove(ZERO_PATH_CACHE);
    }
    ...
    private static function _Get_Index_Path($source, $id, $index)
    {
        $path = $source;
        if ( null == self::$_Memcache )
            $path .= '/' . Zero_System_File::Get_Path_Cache($id);
