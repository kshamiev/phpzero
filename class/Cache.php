...
class Zero_Cache
{
    ...
    public static function Set_Data($index, $value, $time = 0)
    {
        ...
        {
            $index = ZERO_PATH_CACHE . '/' . $index . '.data';
            Zero_Helper_FileSystem::File_Save($index, serialize($value));
        }
        //  Memcache
        else
        {
        ...
        if ( 0 < count(self::$_Link) )
        {
            ...
            {
                $path = ZERO_PATH_CACHE . '/' . $arr[0] . '/' . Zero_Helper_FileSystem::Get_Path_Cache($arr[1]) . '/' . $arr[1] . '/cache.cache';
                Zero_Helper_FileSystem::File_Save_After($path, $index);
            }
            self::$_Link = [];
        }
        return true;
    ...
    {
        $path_file = ZERO_PATH_CACHE . '/' . $this->Model->Source . '/' . Zero_Helper_FileSystem::Get_Path_Cache($this->Model->ID) . '/' . $this->Model->ID . '/cache.cache';
    ...
    public static function Reset_All()
    {
        ...
        //  file cache
        Zero_Helper_FileSystem::File_Remove(ZERO_PATH_CACHE);
    }
    ...
    private static function _Get_Index_Path($source, $id, $index)
    {
        $path = $source;
        if ( null == self::$_Memcache )
            $path .= '/' . Zero_Helper_FileSystem::Get_Path_Cache($id);
