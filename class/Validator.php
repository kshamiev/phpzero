        ...
        if ( isset($_FILES[$prop]) && 4 != $_FILES[$prop]['error'] )
        {
            ...
            if ( 'image' == substr($_FILES[$prop]['type'], 0, 5) )
            {
                ...
                if ( $value['X'] || $value['Y'] || $value['R'] )
                {
                    ...
                    if ( false == Zero_System_File::Image_Resize($_FILES[$prop]['tmp_name'], $_FILES[$prop]['tmp_name'] . 'resize', $value['X'], $value['Y'], $value['R']) )
                    {
            ...
            $_FILES[$prop]['name'] = Zero_System_String::Transliteration_FileName($_FILES[$prop]['name']);
