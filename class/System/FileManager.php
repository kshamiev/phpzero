...
class Zero_System_FileManager extends Zero_Controller
{
    ...
    protected function Chunk_FileUpload()
    {
        if ( 4 != $_FILES['FileUpload']['error'] )
        {
            ...
            $filename = Zero_String::Transliteration_FileName($_FILES['FileUpload']['name']);
    ...
    protected function Chunk_FolderAdd()
    {
        ...
        $path = end($this->Params['obj_parent_path']) . '/' . Zero_String::Transliteration_FileName($_REQUEST['FolderName']);
        mkdir($path);
        chmod($path, 0777);
